...
def plot_losses(loss_history): # , loss_labels=None):
    ...
    if "u_history" in loss_history:
        plt.figure()
        plt.semilogy(loss_history["u_history"]["steps"], loss_history["u_history"]["loss"], label=r"$L_u$")
        plt.title(r"Training Loss - $u(x)_{model}$")
    ...
    if "f_history" in loss_history and loss_history["f_history"]["loss"] is not None:
        plt.figure()
        plt.semilogy(loss_history["f_history"]["steps"], loss_history["f_history"]["loss"], label=r"$L_f$")
        plt.title(r"Training Loss - $f(x)_{model}$")
        plt.xlabel("Iterations")
        plt.ylabel(r"$L_f$")
        plt.grid()
        plt.legend()
    ...
    if "a_history" in loss_history:
        ...
        plt.figure()
        if "loss" in a_hist:
            plt.semilogy(steps, a_hist["loss"], label=r"$L$")
        if "pde_loss" in a_hist and len(a_hist["pde_loss"]) > 0:
            plt.semilogy(steps, a_hist["pde_loss"], label=r"$L_{PDE}$")
        if "a_grad_loss" in a_hist and len(a_hist["a_grad_loss"]) > 0:
            plt.semilogy(steps, a_hist["a_grad_loss"], label=r"$L_{|\nabla a|}$")
        plt.title(r"Training Loss - $a(x)_{model}$")
        plt.xlabel("Iterations")
        plt.ylabel(r"$L$")
        plt.grid()
        plt.legend()
...
def plot_1D(x, u_pred, x_obs=None, a_pred=None, u_obs=None, f_obs=None, a_exact=None, f_pred=None, u_exact=None,
    ...
    plt.title(r"Gelernte Lösung und gemessene Werte")
    plt.xlabel(r"$x$")
    plt.ylabel(r"$u(x)$")
    plt.grid()
    plt.legend()
    ...
    # plt.suptitle(r"TF/Keras Implementierung", fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.show()
